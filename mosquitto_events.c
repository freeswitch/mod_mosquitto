...
void event_handler(switch_event_t *event)
{
	...
	if (!(connection = userdata->connection)) {
		...
		if (!(locate_connection(profile, connection->name))) {
			...
			log(SWITCH_LOG_ALERT, "Cannot publish to topic %s because connection %s (profile %s) is not connected\n",
				topic->name, connection->name, profile->name);
			log(SWITCH_LOG_ALERT,
				"Confirm MQTT broker is reachable, then try command: mosquitto enable profile %s connection %s\n",
				profile->name, connection->name);
	...
	if (mosquitto_globals.log.details && profile->log->details) { log(SWITCH_LOG_DEBUG, "event_handler(): %s\n", buf); }
	/*
	 * mosq		A valid mosquitto instance.
	 * mid			Pointer to an int.  If not NULL, the function will set this to the message id of this particular
	 *message. This can be then used with the publish callback to determine when the message has been sent. Note that
	 *although the MQTT protocol doesn’t use message ids for messages with QoS=0, libmosquitto assigns them message ids
	 *so they can be tracked with this parameter. topic		Null terminated string of the topic to publish to.
	...
	if (mosquitto_globals.log.details && profile->log->details) {
		log(SWITCH_LOG_DEBUG,
			"profile %s connection %s publisher %s topic %s event %s message id %d queued payload: %s\n", profile->name,
			connection->name, publisher->name, topic->pattern, event_name, topic->mid, payload_string);
	} else {
	...
	if (profile->log->details) {
		profile_log(SWITCH_LOG_INFO, profile,
					"profile %s connection %s publisher %s topic %s event %s message id %d queued payload %s\n",
					profile->name, connection->name, publisher->name, topic->pattern, event_name, topic->mid,
					payload_string);
	} else {
		profile_log(SWITCH_LOG_INFO, profile,
					"profile %s connection %s publisher %s topic %s event %s message id %d queued\n", profile->name,
					connection->name, publisher->name, topic->pattern, event_name, topic->mid);
	}
						   ...
						   mosquitto_event_t *event)
{
	...
	if (event->bound) {
		log(SWITCH_LOG_WARNING, "Event already bound: profile %s publisher %s topic %s event %s (%d)\n", profile->name,
			publisher->name, topic->name, event->name, (int)event->event_type);
		return SWITCH_STATUS_GENERR;
	}
	...
	if ((switch_event_bind_removable(event->event_id, event->event_type, SWITCH_EVENT_SUBCLASS_ANY, event_handler,
									 userdata, &event->node)) != SWITCH_STATUS_SUCCESS) {
