...
do
	...
	corefilesearch=/cores/core.*.!*!$testname.* ;
	echo $corefilesearch ;
	if ls $corefilesearch 1> /dev/null 2>&1; then
	    ...
	    gdb -ex "set logging file $LOG_DIR/backtrace_${testname}.txt" -ex "set logging on" -ex "set pagination off" -ex "bt full" -ex "bt" -ex "info threads" -ex "thread apply all bt" -ex "thread apply all bt full" -ex "quit" test/$testname $coredump ;
            ls -la $LOG_DIR
	fi ;
