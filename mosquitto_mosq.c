...
switch_status_t mosq_connect(mosquitto_connection_t *connection)
{
	...
	if (connection->tls.enable) {
		...
		if (connection->tls.port) { port = connection->tls.port; }
	}
	...
	if (!connection->bind_address) {
		...
		if (!connection->srv) {
			 ...
			 */
			rc = mosquitto_connect_bind(connection->mosq, connection->host, port, connection->keepalive,
										connection->bind_address);
		} else {
			rc = mosquitto_connect_srv(connection->mosq, connection->host, connection->keepalive,
									   connection->bind_address);
		}
	}
	switch (rc) {
	case MOSQ_ERR_SUCCESS:
		log(SWITCH_LOG_DEBUG, "Attempting to connect to profile %s %s:%d keepalive:%d bind_address:%s SRV: %s\n",
			connection->profile_name, connection->host, port, connection->keepalive, connection->bind_address,
			connection->srv ? "enabled" : "disabled");
		status = SWITCH_STATUS_SUCCESS;
		break;
	case MOSQ_ERR_INVAL:
		log(SWITCH_LOG_ERROR,
			"Failed connection to profile %s %s:%d keepalive:%d bind_address:%s SRV: %s with invalid parameters\n",
			connection->profile_name, connection->host, port, connection->keepalive, connection->bind_address,
			connection->srv ? "enabled" : "disabled");
		mosq_destroy(connection);
		return SWITCH_STATUS_GENERR;
	...
	default:
		log(SWITCH_LOG_ERROR,
			"Failed connection to profile %s %s:%d keepalive:%d bind_address: %s SRV: %s unknown return code %d\n",
			connection->profile_name, connection->host, port, connection->keepalive, connection->bind_address,
			connection->srv ? "enabled" : "disabled", rc);
		mosq_destroy(connection);
		return SWITCH_STATUS_GENERR;
	}

	loop = mosquitto_loop_start(connection->mosq);
	if (loop != MOSQ_ERR_SUCCESS) { log(SWITCH_LOG_ERROR, "Unable to start loop %i\n", loop); }
...
switch_status_t mosq_loop_stop(mosquitto_connection_t *connection, switch_bool_t force)
{
	...
	case MOSQ_ERR_INVAL:
		log(SWITCH_LOG_INFO,
			"Shutting down profile %s connection: %s mosquitto_loop_stop() %d input parameters were invalid\n",
			connection->profile_name, connection->name, rc);
		status = SWITCH_STATUS_GENERR;
		break;
	case MOSQ_ERR_NOT_SUPPORTED:
		log(SWITCH_LOG_INFO,
			"Shutting down profile %s connection: %si mosquitto_loop_stop() %d thread support is not available\n",
			connection->profile_name, connection->name, rc);
		status = SWITCH_STATUS_GENERR;
		break;
	}
 ...
 *
 * @details This routine dssconnects a connection, stops the mosquitto send/receive loop and clears the pointer to the
 * mosquitto client structure
 *
 * @param[in]   *connection	Pointer to a connection structure
...
switch_status_t mosq_disconnect(mosquitto_connection_t *connection, switch_bool_t force_loop_stop)
{
	...
	if (!connection->connected) {
		...
		case MOSQ_ERR_INVAL:
			log(SWITCH_LOG_DEBUG,
				"Disconnection for profile %s connection %s returned: input parameters were invalid \n",
				connection->profile_name, connection->name);
			status = SWITCH_STATUS_GENERR;
			break;
		case MOSQ_ERR_NO_CONN:
			log(SWITCH_LOG_DEBUG,
				"Tried to disconnect profile %s connection %s but there was no connection to the broker\n",
				connection->profile_name, connection->name);
			connection->connected = SWITCH_FALSE;
			status = SWITCH_STATUS_GENERR;
			break;
...
switch_status_t mosq_new(mosquitto_profile_t *profile, mosquitto_connection_t *connection)
{
	...
	if (!profile->enable || !connection->enable) {
		log(SWITCH_LOG_DEBUG, "mosq_new() Profile %s %s connection %s %s\n", profile->name,
			profile->enable ? "enabled" : "disabled", connection->name, connection->enable ? "enabled" : "disabled");
		return SWITCH_STATUS_SUCCESS;
	}

	if (!(userdata =
			  (mosquitto_mosq_userdata_t *)switch_core_alloc(profile->pool, sizeof(mosquitto_mosq_userdata_t)))) {
		log(SWITCH_LOG_CRIT,
			"mosq_new() Failed to allocate memory for mosquitto_new() userdata structure profile %s connection %s\n",
			profile->name, connection->name);
		return SWITCH_STATUS_GENERR;
	}
	...
	if (connection->client_id != NULL) {
		...
		log(SWITCH_LOG_INFO,
			"mosquitto_new() profile %s connection %s called with NULL client_id, forcing clean_session to TRUE\n",
			profile->name, connection->name);
	}
	...
	/*
	 * id				String to use as the client id.  If NULL, a random client id will be generated.  If id is NULL,
	 *clean_session must be true. clean_session	Set to true to instruct the broker to clean all messages and
	 *subscriptions on disconnect, false to instruct it to keep them. See the man page mqtt(7) for more details.  Note
	 *that a client will never discard its own outgoing messages on disconnect. Calling mosquitto_connect or
	 *mosquitto_reconnect will cause the messages to be resent. Use mosquitto_reinitialise to reset a client to its
	 *original state.  Must be set to true if the id parameter is NULL. obj				A user pointer that will be
	 *passed as an argument to any callbacks that are specified.
	 */
	...
	if (connection->mosq == NULL) {
		...
		default:
			log(SWITCH_LOG_ERROR, "mosquitto_new(%s, %d, NULL) errno(%s)\n", connection->client_id,
				connection->clean_session, mosquitto_strerror(errno));
			return SWITCH_STATUS_GENERR;
		}
	}
							   ...
							   mosquitto_topic_t *topic)
{
	...
	if (!connection->mosq) {
		if (connection->enable) {
			...
			log(SWITCH_LOG_ERROR, "Cannot subscribe to topic %s because connection %s (profile %s) mosq is NULL\n",
				topic->name, connection->name, profile->name) return SWITCH_STATUS_GENERR;
		}
	}

	/*
	 * mosq		A valid mosquitto instance.
	 *	mid			A pointer to an int.  If not NULL, the function will set this to the message id of this particular
	 *message. This can be then used with the subscribe callback to determine when the message has been sent. sub
	 *The subscription pattern. qos			The requested Quality of Service for this subscription.
	 */
	...
	case MOSQ_ERR_SUCCESS:
		...
		mosquitto_log(SWITCH_LOG_DEBUG,
					  "Profile %s connection %s subscriber %s topic %s pattern %s message id %d queued\n",
					  profile->name, connection->name, subscriber->name, topic->name, topic->pattern, topic->mid);
		profile_log(SWITCH_LOG_DEBUG, profile,
					"Profile %s connection %s subscriber %s topic %s pattern %s message id %d queued\n", profile->name,
					connection->name, subscriber->name, topic->name, topic->pattern, topic->mid);
		break;
	case MOSQ_ERR_INVAL:
		log(SWITCH_LOG_ERROR,
			"Profile %s subscriber %s connection %s topic %s pattern %s the input parameters were invalid\n",
	...
	case MOSQ_ERR_NOMEM:
		log(SWITCH_LOG_CRIT,
			"Profile %s subscriber %s connection %s topic %s pattern %s an out of memory condition occurred\n",
			profile->name, subscriber->name, connection->name, topic->name, topic->pattern);
		topic->subscribed = SWITCH_FALSE;
		return SWITCH_STATUS_GENERR;
		break;
	case MOSQ_ERR_NO_CONN:
		log(SWITCH_LOG_WARNING,
			"Profile %s subscriber %s connection %s topic %s pattern %s not connected to an MQTT broker\n",
			profile->name, subscriber->name, connection->name, topic->name, topic->pattern);
		topic->subscribed = SWITCH_FALSE;
		return SWITCH_STATUS_GENERR;
		break;
	case MOSQ_ERR_MALFORMED_UTF8:
		log(SWITCH_LOG_ERROR,
			"Profile %s subscriber %s connection %s topic %s pattern %s the pattern is not valid UTF-8\n",
			profile->name, subscriber->name, connection->name, topic->name, topic->pattern);
		topic->subscribed = SWITCH_FALSE;
		return SWITCH_STATUS_GENERR;
		break;
	...
	case MOSQ_ERR_OVERSIZE_PACKET:
		log(SWITCH_LOG_ERROR,
			"Profile %s subscriber %s connection %s topic %s pattern %s the pattern larger than the MQTT broker can "
			"support\n",
			profile->name, subscriber->name, connection->name, topic->name, topic->pattern);
		topic->subscribed = SWITCH_FALSE;
		return SWITCH_STATUS_GENERR;
						  ...
						  int rc)
{
	...
	case MOSQ_ERR_INVAL:
		...
		if (connection->enable) {
			log(SWITCH_LOG_WARNING,
				"Event handler: failed to publish to %s %s %s trying to initialize the connection\n", profile->name,
				connection->name, topic->pattern);
			connection_initialize(profile, connection);
		}
		break;
	case MOSQ_ERR_NOMEM:
	...
	case MOSQ_ERR_PROTOCOL:
		log(SWITCH_LOG_DEBUG,
			"Event handler: failed to publish to %s %s %s protocol error communicating with the broker\n",
			profile->name, connection->name, topic->pattern);
		break;
	case MOSQ_ERR_PAYLOAD_SIZE:
...
switch_status_t mosq_startup(void)
{
	...
	status = switch_file_open(&mosquitto_globals.log.logfile, mosquitto_globals.log.name,
							  SWITCH_FOPEN_WRITE | SWITCH_FOPEN_APPEND | SWITCH_FOPEN_CREATE, SWITCH_FPROT_OS_DEFAULT,
							  mosquitto_globals.pool);
